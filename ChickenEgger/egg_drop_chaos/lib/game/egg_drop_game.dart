...
class EggDropGame extends FlameGame with TapCallbacks, HasCollisionDetection {
  ...
  void update(double dt) {
    ...
    if (chicken.position.y > size.y - ground.groundHeight) {
      _triggerGameOver();
    }
  }
  ...
  double _getTargetSpawnInterval() {
    ...
    return (2.0 - (_gameSpeed - AppConfig.gameSpeed) / 200).clamp(0.8, 2.0);
  }
    ...
    for (final egg in _eggs) {
      ...
      if (egg.position.y > size.y - ground.groundHeight - 10) {
        if (!egg.hasHit) {
          ...
          gameStateService.addScore(AppConfig.pointsPerMiss);
          gameStateService.resetCombo();
        }
        toRemove.add(egg);
        continue;
      }
